...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, class_num, shot_num_per_class):
    ...
    for iSample in range(da_nTrain):
        imdb_da_train['data'][:, :, :, iSample] = utils.DCLFSL_radiation_noise(
...
class D_Res_3d_CNN(nn.Module):
    def __init__(self, in_channel, out_channel1, out_channel2):
        ...
        import Pconv
        self.Pconv1 = Pconv.PConv(100, 144, k=3, s=1)
        self.Pconv2 = Pconv.PConv(144, 144, k=3, s=1)
        self.Pconv3 = Pconv.PConv(144, 100, k=3, s=1)
...
class Network(nn.Module):
    def __init__(self,FEATURE_DIM,CLASS_NUM,TAR_INPUT_DIMENSION,SRC_INPUT_DIMENSION,N_DIMENSION):
        ...
        self.classifier1 = nn.Linear(in_features=self.final_feat_dim, out_features=1)
    ...
    def forward(self, x, domain='source'):  # x
        ...
        outputs = self.classifier1(feature)
        outputs = self.sigmoid(outputs)
        return feature, output,  outputs
...
def weights_init(m):
    ...
    # 针对 PConv 的特殊处理
    if classname == 'PConv':
        if hasattr(m, 'cw') and hasattr(m.cw, 'conv') and isinstance(m.cw.conv, nn.Conv2d):
            nn.init.xavier_uniform_(m.cw.conv.weight, gain=1)
            if m.cw.conv.bias is not None:
                m.cw.conv.bias.data.zero_()

        if hasattr(m, 'ch') and hasattr(m.ch, 'conv') and isinstance(m.ch.conv, nn.Conv2d):
            nn.init.xavier_uniform_(m.ch.conv.weight, gain=1)
            if m.ch.conv.bias is not None:
                m.ch.conv.bias.data.zero_()

        if hasattr(m, 'cat') and hasattr(m.cat, 'conv') and isinstance(m.cat.conv, nn.Conv2d):
            nn.init.xavier_uniform_(m.cat.conv.weight, gain=1)
            if m.cat.conv.bias is not None:
                m.cat.conv.bias.data.zero_()
        return

    # 通用卷积层初始化
    elif classname.find('Conv') != -1:
        if hasattr(m, 'weight') and m.weight is not None:  # Ensure it has weight attribute
...
def DC_IP():
    ...
    parser = argparse.ArgumentParser(description="DCLFSL")
    ...
    seeds = [1332, 1230, 1336, 1337, 1240, 1236, 1226, 1235, 1233, 1229]
    for iDataSet in range(nDataSet):
        ...
        # model
        feature_encoder = Network(FEATURE_DIM,CLASS_NUM,TAR_INPUT_DIMENSION,SRC_INPUT_DIMENSION,N_DIMENSION)
        ...
        for episode in range(1):  # EPISODE = 90000
            ...
            if episode % 2 == 0:
                ...
                # calculate features
                support_features, support_outputs,_ = feature_encoder(supports.cuda())  # torch.Size([409, 32, 7, 3, 3])
                query_features, query_outputs,_ = feature_encoder(querys.cuda())  # torch.Size([409, 32, 7, 3, 3])
                target_features, target_outputs,_ = feature_encoder(target_data.cuda(), domain='target')  # torch.Size([409, 32, 7, 3, 3])
            ...
            # target domain few-shot + domain adaptation
                ...
                # calculate features
                support_features, support_outputs,_ = feature_encoder(supports.cuda(),  domain='target')  # torch.Size([409, 32, 7, 3, 3])
                query_features, query_outputs,_ = feature_encoder(querys.cuda(), domain='target')  # torch.Size([409, 32, 7, 3, 3])
                source_features, source_outputs,_ = feature_encoder(source_data.cuda())  # torch.Size([409, 32, 7, 3, 3])

                task_cl = utils.Task(target_da_metatrain_data, CLASS_NUM, 2, QUERY_NUM_PER_CLASS)  # 5， 1，15
                support_dataloader_cl = utils.get_HBKC_data_loader(task_cl, num_per_class=2, split="train",
                                                                shuffle=False)
                supports_cl, support_labels = next(support_dataloader_cl.__iter__()) # (5, 100, 9, 9)

                supports_1 = supports_cl[0:18:2, :, :, :]
                supports_2 = supports_cl[1:18:2, :, :, :]

                support_features_1, support_output_1, support_sigmoid_1 = feature_encoder(supports_1.cuda(),  domain='target')
                support_features_2, support_output_2, support_sigmoid_2 = feature_encoder(supports_2.cuda(),  domain='target')

                import DCL
                cl_fn = DCL.DCL(temperature=0.1)
                cl_loss = cl_fn(support_sigmoid_1,support_sigmoid_2)
                ...
                loss = f_loss + domain_loss + cl_loss
            ...
            if (episode + 1) % 100 == 0 or episode == 0:
                ...
                for test_datas, test_labels in test_loader:
                    ...
                    test_features, _ ,_= feature_encoder(Variable(test_datas).cuda(), domain='target')  # (100, 160)
                ...
                if test_accuracy > last_accuracy:
                    # save networks
                    # torch.save(feature_encoder.state_dict(),str("CLFSL_feature_encoder_" + "IP_" +str(iDataSet) +"iter_" + str(TEST_LSAMPLE_NUM_PER_CLASS) +"shot.pkl"))
    ...
    for i in range(CLASS_NUM):
        ...
        append_logs("Class " + str(i) + ": " + "{:.2f}".format(100 * AMean[i]) + " +- " + "{:.2f}".format(100 * AStd[i]))
    ...
    image_path = "classificationMap/IP_{}shot.png".format(TEST_LSAMPLE_NUM_PER_CLASS)
